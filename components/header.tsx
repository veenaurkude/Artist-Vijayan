...
export default function Header() {
  ...
  return (
    <header
    //  data-aos="fade-down"
      className={`${styles.header} ${scrolled ? styles.scrolled : ""} fixed w-full z-50 transition-all duration-300`}
    >
...
<Link href="/" className={styles.logo}>
  ...
  <ColorText>The Art of Vijayan</ColorText>
        ...
        <nav className="hidden md:flex space-x-8">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`${styles.navLink} ${pathname === link.href ? styles.active : ""} hoverable relative `}
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="md:hidden flex items-center">
          <button onClick={toggleMenu} className="hoverable" aria-label="Toggle menu">
